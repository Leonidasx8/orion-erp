...
export default async function CreditoPage() {
  ...
  const [totalesRaw, agingRaw, clientesRaw] = await Promise.all([
    ...
    db.execute<{
      ...
      SELECT
        ...
        COALESCE(SUM(bucket_90_mas), 0)::text AS bucket_90_mas
      FROM aging_cxc
      WHERE tenant_id = ${tenant.id}
    `),
    ...
    db.execute<{
      ...
      SELECT
        ...
        cxc.saldo_total,
        cxc.saldo_vencido,
        cxc.dias_mas_vencido::text           AS dias_mas_vencido,
  ...
  const agingBuckets: AgingBuckets = {
    ...
    bucket90mas: Number(agingRow.bucket_90_mas),
  };
